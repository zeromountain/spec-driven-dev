...
| 제약 | 왜 |
|---|---|
| `enforce: true`면 **같은 페이즈끼리만** 동시에 돈다 | 페이즈 게이트는 프로젝트 전역이다. 훅의 stdin에 호출자 정보가 없어 "지금 이 프로젝트가 어떤 페이즈인가" 하나만 알 수 있다 |
| 구현 단계는 **파일이 겹치면** 직렬화된다 | `impl-planner`의 `tasks[].files`로 교집합을 본다. 계획이 없으면(경량 모드) "모른다 = 겹칠 수 있다"로 보고 직렬화한다 |

현재 페이즈에서 아무도 못 움직이면 가장 오래 기다린 파이프라인의 단계로 페이즈가 넘어간다 —
굶지 않는다. 깊은 모드에서 파일 범위가 서로 다르면 두 기능의 구현이 진짜로 동시에 돈다.
