...
걸어두지 않는다. 작성·계획·구현·리뷰를 서로 다른 서브에이전트에게 맡기고, 프로젝트가
원하면 PreToolUse 훅으로 페이즈 경계를 실제로 집행한다.
...
핵심 분리는 **작성과 검토**, 그리고 **관심사별 리뷰어**다. 리뷰어 하나가 코드 품질·보안·
성능을 한꺼번에 보면 앞의 관심사가 뒤의 것을 덮는다. 리뷰어들은 서로의 판정을 보지 못한
채 한 번에 호출되고, **하나라도 `changes-requested`면 전체가 그렇다**(평균 내지 않는다).
**명세 준수·AC 커버리지·스펙 밖 구현을 전담 판정하는 리뷰어는 없다** — `code-reviewer`가
review 단계의 유일한 기본값이고, 인수 기준 커버리지·게이트 위반 표는 리포트에 자동으로
실리지만 그걸 근거로 승인/반려를 판정하는 사람은 없다.

## 깊이 — 작은 변경에 6명을 부르지 않는다
...
경우다. 스캔에서 명세의 `범위 밖` 섹션과 미기입 플레이스홀더는 제외한다(거기 적힌 "인증은
다루지 않는다"는 신호가 아니라 그 반대이고, 템플릿 안내문은 명세의 내용이 아니다).
...
/sdd:run 사용자 엔티티에 결혼여부 필드 추가    # 명세→(승인)→구현→리뷰→(회고)
/sdd:run 결혼여부 필드 추가 --no-gate          # 승인 지점 없이 끝까지
...
1. **명세** — `spec-architect`가 `docs/sdd/`(PRD·아키텍처·ADR)와 기존 명세를 읽고 명세를 쓴다.
2. **👤 명세 승인** — 인수 기준·오류 케이스·범위 밖·가정 요약을 보고 승인하거나, 고칠 점을
   말하면 같은 명세에 반영돼 다시 돌아온다. (기본 켜짐)
3. **계획** — 깊은 모드면 `impl-planner`가 태스크와 태스크별 **검증 커맨드**를 정한다.
   (👤 계획 승인은 `humanGates.plan`으로 켠다)
4. **구현** — `software-engineer`가 구현·테스트·검증 커맨드를 실행한다. 실패하면 다시.
5. **리뷰** — 리뷰어들이 동시에 판정한다. 반려되면 지적을 들고 구현으로.
6. **👤 회고** — 스크립트가 센 재시도·막힌 내용을 보고 남길 교훈을 고른다. 고른 교훈은
   `docs/sdd/learnings.md`에 쌓여 **다음 기능의 모든 에이전트 컨텍스트에 실린다.**

사람이 개입하는 자리는 👤뿐이다. 나머지 단계 사이에서는 묻지 않는다. 확인 없이 끝까지
돌리려면 `--no-gate`. 단계별 안내와 체크리스트는
[`docs/SETUP.md`의 "사용 흐름"](docs/SETUP.md#기능-하나를-끝까지-사용-흐름)에 있다.
...
미결 질문(`ask-user`), 재시도 상한 초과(`halted`) 셋이고, 리뷰 승인 뒤에는 회고(`reflect`)가
한 번 나온다. 단계별 상한은 기본 2회, 전체 전이 상한은 24회다. 전이표·인계 항목·중단 사유는
...
명세 승인·피드백·회고 교훈 고르기는 파이프라인이 멈춰서 물어볼 때 말로 답한다("승인",
"AC-2에 수치 넣어", "1번 교훈만 남겨").
...
|---|---|---|
| 6개 역할 | 서브에이전트가 각자 독립된 컨텍스트에서 | 스킬 하나가 순서대로 직접 수행 |
| 페이즈 게이트 | 훅이 실제로 쓰기를 차단 | 훅 없음 — `sdd.py guard`로 사후 점검 |

그래서 Codex에서는 리뷰어 3종의 독립 판정이 **순차적 자기 점검에 가깝다.** 워크트리는
Codex에서도 파일을 격리하지만, "훅이 파이프라인별로 판정한다"는
