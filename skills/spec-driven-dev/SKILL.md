...
- 리뷰어 판정을 네가 합치지 마라. 종합은 전원이 모였을 때 `advance`가 한다.
- **리뷰어 결과에 `agent` 키를 빠뜨리지 마라.** 로스터가 여럿인데 이름 없는 판정을 넘기면
  파이프라인이 거부한다 — 한 리뷰어가 나머지를 대신 승인해 버리기 때문이다.
...
경로이므로 그대로 쓰면 된다 — 게이트도 `specs/<슬러그>/`의 슬러그로 주인을 찾으므로, 다른
파이프라인이 다른 단계에 있어도 자기 명세는 쓸 수 있다.
...
리뷰어가 늘었거나(플러그인 업그레이드) 특정 단계만 다시 돌리고 싶을 때 쓴다.

```bash
$S/sdd.py run --spec <슬러그> --from review --path <root>   # 명세·구현은 그대로
```

`--restart`와 다르다 — `--restart`는 파이프라인을 새로 만들어 **명세 버전을 올린다.**
`--from`은 기존 파이프라인의 그 단계만 되열고, 로스터를 다시 판정해 적용한다. 리뷰를
다시 열면 리포트 골격도 새로 만든다(낡은 리포트에는 새 리뷰어 절이 없다). 반환값의
`rosterBefore` / `roster`를 비교해 무엇이 달라졌는지 사용자에게 알린다.
