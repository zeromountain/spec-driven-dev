...
`.sdd/worktrees/<슬러그>/...` 경로에는 슬러그가 있다. `sdd.resolve_write()`가 그 경로를
주인 파이프라인에 귀속시키고, 게이트는 전역 `state.phase`가 아니라 그 파이프라인의
`stage`로 판정한다(deny 메시지에 `[슬러그]` 접두어가 붙는다). 여전히 해결되지 않는 것은
**같은 워크트리 안에서의 역할 분리**다 — engineer와 test-engineer는 같은 디렉터리를 쓴다.
