...
`update_state()`(= `.sdd/state.lock` 배타 락 안에서 다시 읽고 → 자기 몫만 고치고 → 저장)를
지나고, `write_json()`은 임시 파일 + `os.replace`로 원자적으로 교체한다. 이게 없으면 동시에
들어온 `advance` 두 개 중 하나가 조용히 사라지고, 쓰는 중에 읽은 쪽은 잘린 JSON을 빈 상태로
읽어 레지스트리를 통째로 날린다.
